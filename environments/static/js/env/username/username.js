...
var ExAsUsername = (function() {
    ...
    var action_btn = function(username = '') {
        return '<div class="input-group">' +
            '<a href="https://www.tiktok.com/@' + username + '" target="_blank"><button type="button" class="btn btn-secondary btn-icon waves-effect waves-light tombolView"><i class="ri-eye-line"></i></button></a>' +
    ...
    var loadParent = () => {
        $.ajax({
            ...
            success: function(response) {
                var res = response
                if (res.status) {
                    var opt = "<option value=''> Pilih Parent </option>"
                    opt += "<option value='all'>All</option>"
                    $.each(res.data, function(i, item) {
                        opt += "<option value='" + item.uname + "'>" + item.uname + "</option>"
                    })
                    $('#parent_select').html(opt).trigger('change')
                }
            }
        })
                    ...
                    $.each(res.data, function(i, item) {
                        ...
                        if (isReady === false) {
                            ...
                            tb.row.add([
                                ...
                                item.is_private,
                                item.from_username,
                                action_btn(item.username)
                            ])
                        } else {
                            tb.rows().every(function(rowIdx, tableLoop, rowLoop) {
                                var data = this.data();
                                if (data[1] == item.id) {
                                    if (data[2] !== item.username) {
                                        tb.cell(rowIdx, 2)
                                            .data(item.username)
                                    }
                                    ...
                                    if (data[9] !== item.from_username) {
                                        tb.cell(rowIdx, 9)
                                            .data(item.from_username)
                                    }

                                    if (data[10] !== action_btn(item.username)) {
                                        tb.cell(rowIdx, 10)
                                            .data(action_btn(item.username))
                                    }
                                }
                            });
                        }
...
var ExAsFresh = ExAsUsername;
ExAs.Dom(ExAsUsername.run())